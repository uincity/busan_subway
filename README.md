...
처음 서비스를 사용하는 사용자는 [웹서비스 사용자 보고서·기능 명세](WEB_SERVICE_USER_GUIDE.md)를 먼저 확인하세요.
...
`아파트연결` 조회는 유효한 SQLite 결과를 우선 읽고, 현재 조회 조건의 미계산·갱신 필요 항목만
화면에서 자동 계산합니다. 완료 결과는 일반 재실행이나 지도 조작에서 다시 호출하지 않습니다.
운영자가 초기 결과를 대량으로 준비하려면 GraphHopper 키를 설정한 뒤 기본 역인 대연역
(역 ID `213`, 2호선)부터 사전 계산할 수 있습니다.
